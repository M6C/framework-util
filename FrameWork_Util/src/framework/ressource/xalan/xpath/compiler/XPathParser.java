...
// Source File Name:   XPathParser.java
...
import framework.ressource.xalan.xpath.XPathException;
import framework.ressource.xalan.xpath.XPathProcessorException;
import framework.ressource.xalan.xpath.domapi.XPathStylesheetDOM3Exception;
...
public class XPathParser
{
        ...
        throws TransformerException
    {
        ...
        try
        {
            ...
            if(null != m_token)
            {
                ...
                error("ER_EXTRA_ILLEGAL_TOKENS", new Object[] {
                    extraTokens
                });
            }
        }
    ...
    {
        return m_token == null ? s == null : m_token.equals(s);
    }

    final boolean tokenIs(char c)
    {
        return m_token == null ? false : m_tokenChar == c;
    }
    ...
    final boolean lookahead(char c, int n)
    {
        ...
        if(pos <= m_ops.getTokenQueueSize() && pos > 0 && m_ops.getTokenQueueSize() != 0)
        {
            ...
            b = tok.length() != 1 ? false : tok.charAt(0) == c;
        } else
        {
            b = false;
        }
    ...
    private final boolean lookahead(String s, int n)
    {
        ...
        if(m_queueMark + n <= m_ops.getTokenQueueSize())
        {
            ...
            isToken = lookahead == null ? s == null : lookahead.equals(s);
        } else
        {
            isToken = null == s;
        }
        ...
        throws TransformerException
    {
        ...
        {
            error("ER_EXPECTED_BUT_FOUND", new Object[] {
                expected, m_token
            });
    ...
    protected String dumpRemainingTokenQueue()
    {
        ...
        if(q < m_ops.getTokenQueueSize())
        {
            String msg;
            String t;
            for(msg = "\n Remaining tokens: ("; q < m_ops.getTokenQueueSize(); msg = msg + " '" + t + "'")
                t = (String)m_ops.m_tokenQueue.elementAt(q++);
        ...
        throws TransformerException
    {
        ...
        } else
        {
            ...
            if(-1 == funcTok)
                error("ER_COULDNOT_FIND_FUNCTION", new Object[] {
                    m_token
                });
            switch(funcTok)
            {
        ...
        throws TransformerException
    {
        ...
        if(m_token != null && !RelativeLocationPath() && !seenSlash)
            error("ER_EXPECTED_LOC_PATH", new Object[] {
                m_token
            });
        m_ops.setOp(m_ops.getOp(1), -1);
        m_ops.setOp(1, m_ops.getOp(1) + 1);
        ...
        throws TransformerException
    {
        ...
        if(tokenIs('*') || tokenIs('@') || tokenIs('_') || m_token != null && Character.isLetter(m_token.charAt(0)))
        {
            Basis();
            for(; tokenIs('['); Predicate());
            m_ops.setOp(opPos + 1, m_ops.getOp(1) - opPos);
        ...
        throws TransformerException
    {
        ...
        if(null == val)
            error("ER_ILLEGAL_AXIS_NAME", new Object[] {
                m_token
            });
        ...
        throws TransformerException
    {
        if(lookahead('(', 1))
        {
            ...
            if(null == nodeTestOp)
            {
                error("ER_UNKNOWN_NODETYPE", new Object[] {
                    m_token
                });
            } else
            {
        ...
        throws TransformerException
    {
        ...
        {
            error("ER_PATTERN_LITERAL_NEEDS_BE_QUOTED", new Object[] {
                m_token
            });
        }
    }
        ...
        throws TransformerException
    {
        if(null != m_token)
        {
            ...
            {
                num = 0.0D;
                error("ER_COULDNOT_BE_FORMATTED_TO_NUMBER", new Object[] {
                    m_token
                });
            }
    ...
    protected void RelativePathPattern()
        throws TransformerException
    {
        for(boolean trailingSlashConsumed = StepPattern(false); tokenIs('/'); trailingSlashConsumed = StepPattern(!trailingSlashConsumed))
            nextToken();
        ...
        throws TransformerException
    {
        ...
        if(lookahead("::", 1))
        {
            ...
            {
                axesType = -1;
                error("ER_AXES_NOT_ALLOWED", new Object[] {
                    m_token
                });
            }
            nextToken();
