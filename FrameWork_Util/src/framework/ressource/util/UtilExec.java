...
/**
 * A class that eases the pain of running external processes from applications. Lets you run a program three ways: <OL> <LI><B>exec</B>: Execute the command, returning immediately even if the command is still running. This would be appropriate for printing a file. <LI><B>execWait</B>: Execute the command, but don't return until the command finishes. This would be appropriate for sequential commands where the first depends on the second having finished (e.g. <CODE>javac</CODE> followed by <CODE>java</CODE>). <LI><B>execPrint</B>: Execute the command and print the output. This would be appropriate for the UNIX command <CODE>ls</CODE>. </OL> Note that the PATH is not taken into account, so  you must specify the <B>full</B> pathname to the command, and shell builtin commands will not work. For instance, on Unix the above three examples might look like: <OL> <LI><PRE>UtilExec.exec("/usr/ucb/lpr Some-File");</PRE> <LI><PRE> UtilExec.execWait("/usr/local/bin/javac Foo.java"); UtilExec.execWait("/usr/local/bin/java Foo"); </PRE> <LI><PRE>UtilExec.execPrint("/usr/bin/ls -al");</PRE> </OL>
...
public class UtilExec {
  ...
  /**
 * Determines if the UtilExec class should print which commands are being executed, and print error messages if a problem is found. Default is true.
 ...
 */
  ...
  /**
 * Will UtilExec print status messages?
 * @uml.property  name="verbose"
 */
 ...
 */
  ...
  public static void setOutput(OutputStream outputFlag) {
    output = outputFlag;
  }
